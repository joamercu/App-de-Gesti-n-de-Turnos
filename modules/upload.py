                ...
                for i, row in df.iterrows():
                    ...
                    if not tarea_actual.empty:
                        ...
                        if estado_anterior != estado_nuevo or avance_anterior != avance_nuevo:
                            cambios.append({
                                "ID": row["ID"],
                                "Grupo": tarea_actual.iloc[0]["Grupo"],
                                "Responsable": tarea_actual.iloc[0]["Responsable"],
                    ...
                    try:
                        ...
                        for i, row in df.iterrows():
                            ...
                            if not idx.empty:
                                ...
                                for campo in ["Estado", "% Avance", "Responsable", "Observaciones"]:
                                    if campo in df.columns and not pd.isna(row[campo]):
                                        df_tareas_actual.loc[idx, campo] = row[campo]

                                # Actualizar fecha fin si el estado es Completada
                                if row["Estado"] == "Completada":
                                    # Verificar si la fecha fin es NaN usando pandas.isna() para cada celda individual
                                    fecha_fin = df_tareas_actual.loc[idx[0], "Fecha fin"]
                                    if pd.isna(fecha_fin):
                                        df_tareas_actual.loc[idx[0], "Fecha fin"] = datetime.now().strftime("%Y-%m-%d")

                                # Actualizar historial
                                if len(idx) > 0:  # Verificar que idx no esté vacío
                                    historial_actual = df_tareas_actual.loc[
                                        idx[0], "Historial"] if "Historial" in df_tareas_actual.columns else None

                                    # Inicializar historial si no existe o es nulo
                                    if historial_actual is None or pd.isna(historial_actual):
                                        historial = []
                                    else:
                                        try:
                                            historial = json.loads(historial_actual)
                                        except:
                                            historial = []  # Si hay error al parsear JSON
                                    ...
                                    df_tareas_actual.loc[idx[0], "Historial"] = json.dumps(historial)
